...
export async function killCommand(taskId: string, options: { json?: boolean }): Promise<void> {
  ...
  try {
    const detail = await client.killTask(taskId);
    ...
    if (task) {
      success(`Task ${task.id} killed. Status: ${task.status}`);
    } else {
      success('Task kill request sent.');
    }
  } catch (err) {
